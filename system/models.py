...
class Product(CoreModel):
    name = models.CharField(max_length=255, unique=True, verbose_name="产品名字", help_text="产品名字")
...
class ChipType(CoreModel):
    product_id = models.ForeignKey('Product', on_delete=models.CASCADE)
    name = models.CharField(max_length=255, unique=True, verbose_name="芯片类型", help_text="芯片类型")
    status = models.BooleanField(default=True, verbose_name="是否启用", help_text="是否启用")

    class Meta:
        db_table = 'system_chiptype'
        verbose_name = '芯片类型'
        verbose_name_plural = verbose_name
        ordering = ('-id',)
...
class Category(CoreModel):
    chipType_id = models.ForeignKey('ChipType', on_delete=models.CASCADE)
    product_id = models.ForeignKey('Product', on_delete=models.CASCADE)
    name = models.CharField(max_length=255, unique=True, verbose_name="种类", help_text="种类")
    status = models.BooleanField(default=True, verbose_name="是否启用", help_text="是否启用")
    class Meta:
...
class Version(CoreModel):
    ...
    chipType_id = models.ForeignKey('ChipType', on_delete=models.CASCADE)
    product_id = models.ForeignKey('Product', on_delete=models.CASCADE)
    name = models.CharField(max_length=255, unique=True, verbose_name="版本", help_text="版本")
    status = models.BooleanField(default=True, verbose_name="是否启用", help_text="是否启用")
...
class ScheduledTasks(CoreModel):
    instanceId = models.CharField(verbose_name="实例id", max_length=128)
    changeData = models.CharField(verbose_name="变更数据值", max_length=1024)
    tasksType = models.CharField(verbose_name="任务类型", max_length=64)

    class Meta:
        db_table = "system_scheduled_tasks"
        verbose_name = '定时任务'
        verbose_name_plural = verbose_name
        ordering = ('-create_datetime',)


class SupportCase(CoreModel):
    caseId = models.CharField(verbose_name="caseid",max_length=128)
    caseTitle = models.TextField(verbose_name="case标题")
    content = models.TextField(verbose_name="内容")
    status = models.CharField(verbose_name="状态",max_length=54)
    dept = models.ForeignKey(to='Dept', verbose_name='所属部门', on_delete=models.SET_NULL, db_constraint=False,
                             null=True,
                             blank=True, help_text="关联部门")
    creator = models.CharField(verbose_name="创建者",max_length=128,null=False)

    class Meta:
        db_table = "system_support_case"
        verbose_name = '客户反馈问提'
        verbose_name_plural = verbose_name
        ordering = ('-create_datetime',)